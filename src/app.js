...
/* pavan
var isDisconnect;
const closeTakebotConfirmationAlert = () => {
    setTimeout(() => {
        takeBotMsg(
            "takeda-mi-chatbot-bot-msg takeda-mi-chatbot-hcp",
            closeTakebotTemplate,
            500,
            true,
            "html"
        );
...
const closeTakebotConfirmationRes = (isYes) => {
    if (isYes) {
        isDisconnect = true;
        closeTakebot();
    }
...
const closeTakebot = () => {
    if (isDisconnect) {
        ...
        document.getElementById("takeda-mi-chatbot-chat-outside-icon").style.display = "block";
    }
    else { closeTakebotConfirmationAlert(); }
};
*/
const closeTakebot = () => {
    const closeBot = () => {
        ...
        if (window.location.href.toLowerCase().indexOf("contacttakedamedicalaffairs") > 0)
            document.getElementById("takeda-mi-chatbot-chat-outside-icon").style.display = "none";
    };
    
    if(isBotOpen) {
        if (confirm("Do you want to end the conversation with Takeda Live Chat?")) closeBot();
    } else {
        closeBot();
    }    
};
...
const openTakeBot = () => {
    clear();
    document.getElementById("takeda-mi-chatbot-container").style.display = "block";
    document.getElementById("takeda-mi-chatbot-chat-outside-icon").style.display = "none";
    revokeUserInputAction();
    startChat();
};
